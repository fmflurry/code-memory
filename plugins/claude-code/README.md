...
| `UserPromptSubmit` | Detects substantive code intent → `code-memory retrieve --json` → emits the formatted Context Pack as `additionalContext` so it lands in the model's view of this turn. 5 min cache, 60 s dedup. Resets the per-turn gate flag (see `PreToolUse`). |
| `PreToolUse` (`Read`/`Bash`/`Grep`/`Glob`) | First-tool gate: if no `codememory_retrieve` (auto or agent-initiated MCP) has fired this turn, emits a one-shot soft nudge as `additionalContext` reminding the agent to query the index before scanning the filesystem. Never blocks. Goes silent once any `codememory_*` MCP tool fires, and once per turn either way. |
